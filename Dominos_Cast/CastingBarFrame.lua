...
function DominosCastingBarMixin:OnLoad(unit, showTradeSkills, showShield)
	self.StagePoints = {};
	self.StagePips = {};
	self.StageTiers = {};
...
function DominosCastingBarMixin:OnShow()
	if ( self.unit ) then
...
function DominosCastingBarMixin:GetEffectiveType(isChannel, notInterruptible, isTradeSkill, isEmpowered)
...
function DominosCastingBarMixin:GetTypeInfo(barType)
	if not barType then
		barType = "standard";
	end
...
function DominosCastingBarMixin:HandleInterruptOrSpellFailed(empoweredInterrupt, event, ...)
...
function DominosCastingBarMixin:HandleCastStop(event, ...)
...
function DominosCastingBarMixin:OnEvent(event, ...)
	local arg1 = ...;
...
function DominosCastingBarMixin:UpdateInterruptibleState(notInterruptible)
...
function DominosCastingBarMixin:OnUpdate(elapsed)
...
function DominosCastingBarMixin:ShowSpark()
	if ( self.Spark ) then
		self.Spark:Show();
	end
...
function DominosCastingBarMixin:HideSpark()
	if ( self.Spark ) then
		self.Spark:Hide();
	end
...
function DominosCastingBarMixin:PlayInterruptAnims()
...
function DominosCastingBarMixin:StopInterruptAnims()
	if self.HoldFadeOutAnim then
...
function DominosCastingBarMixin:PlayFadeAnim()
...
function DominosCastingBarMixin:PlayFinishAnim()
	if not self.playCastFX then
		return;
	end
...
function DominosCastingBarMixin:StopFinishAnims()
	if self.FlashAnim then
		self.FlashAnim:Stop();
	end
...
function DominosCastingBarMixin:StopAnims()
...
function DominosCastingBarMixin:UpdateStage()
	local maxStage = 0;
